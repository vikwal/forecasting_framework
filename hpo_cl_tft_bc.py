    ...
    while completed_trials < base_config['hpo']['trials']:
        ...
        try:
            ...
            if use_cache and cache_id != "no_cache":
                enforce_cache_budget(data_cache.DEFAULT_CACHE_DIR, cache_id,
            ...
            for fold_idx in range(len(lazy_fold_loader)):
                ...
                if not is_multi_objective:
                    trial.report(fold_metrics[objectives[0]['metric']], step=fold_idx)

                metrics_str = ', '.join([f"{k}: {v:.4f}" for k, v in fold_metrics.items()])
                logging.info(f'Processed fold {fold_idx + 1}/{len(lazy_fold_loader)}, {metrics_str}')
