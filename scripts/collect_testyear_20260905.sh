...
# l1: fixed-epoch-Testjahrlaeufe (testmode_fe_*), einmal trainierte HIST-Arme (testmode_*_once)
...
cd "$REPO" || exit 1
echo "== von l1"
for a in testmode_fe_dcrnn testmode_fe_dcrnn_idw_alt testmode_fe_mtgnn_nwp \
         testmode_fe_dcrnn_nwp_hist_s1 testmode_fe_dcrnn_nwp_hist_s2 testmode_fe_dcrnn_nwp_hist_s3 \
         testmode_dcrnn_nwp_hist_once testmode_mtgnn_nwp_hist_once; do
...
ls -la --time-style=+%F\ %H:%M data/raw_preds/ | grep -E "testmode_fe_|_once_raw|testyear_mos|testmode_tft|rep[23]_" | awk '{print "  "$6" "$7" "$8}'
