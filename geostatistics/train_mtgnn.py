"""
train_mtgnn.py — Training script for the MTGNN baseline model.

Reuses data loading from train_stgnn2.py (station measurements, ICON-D2 runs,
imputation, run-pair construction, scaling).

Key differences from train_dcrnn.py
-------------------------------------
  • Homogeneous graph only — no ECMWF, no separate NWP nodes
  • NWP aggregated to station level via inverse-distance-weighted mean (HomoSampler)
  • Curriculum learning: cl_steps grows from 1 → F_h over training
  • No HeteroData / torch_geometric dependency in forward pass

Config section: ``mtgnn``  (analog to ``dcrnn``)

Usage
-----
    python geostatistics/train_mtgnn.py \\
        --config configs/config_wind_mtgnn.yaml \\
        --suffix v1
"""
...
logger = logging.getLogger("train_mtgnn")
...
def _train_epoch(
    ...
    """Returns (weighted_loss, RMSE_phys, R²_phys) over curriculum steps."""
    model.train()
    total_loss = 0.0
    ...
    for step_i in range(steps):
        ...
        all_preds.append(pred[:, :cl_steps].detach().cpu())
        all_gts.append(gt[:, :cl_steps].cpu())
...
def _val_epoch(
    ...
    """Returns (weighted_loss, RMSE_phys, R²_phys) restricted to cl_steps."""
    model.eval()
    total_loss = 0.0
    all_preds: list[torch.Tensor] = []
    all_gts:   list[torch.Tensor] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train MTGNN baseline model")
    ...
    log_name = f"train_mtgnn_{log_stem}_{args.suffix}.log" if args.suffix else f"train_mtgnn_{log_stem}.log"
    ...
    if next_n_ecmwf > 0 and ecmwf_features:
        ...
        if ecmwf_path and os.path.exists(ecmwf_path):
            ...
            _, ecmwf_coords, grid_ecmwf_runs, _ = load_ecmwf_runs_at_stations_and_grid(
                ...
                run_times=run_times, freq_h=data_cfg.get("freq", "1h"),
                horizon=F_h,
    ...
    model = MTGNNModel(
        ...
        # Defaults reproduce every earlier run bit for bit (attention).
    ...
    logger.info("MTGNN parameters: %s", f"{model.count_parameters():,}")
    ...
    # Curriculum learning schedule: cl_period epochs per additional step
    cl_period = mcfg.get("cl_period", 3)   # increase cl_steps every N epochs
    ...
    for epoch in range(1, max_epochs + 1):
        # Curriculum: cl_steps grows from 1 to F_h over training
        cl_steps = min((epoch - 1) // cl_period + 1, F_h)

        # Reset patience every time the curriculum horizon extends so early
        # stopping cannot fire while output neurons are still being trained.
        if cl_steps > prev_cl_steps:
            no_improve  = 0
            best_val    = float("inf")
            prev_cl_steps = cl_steps
        ...
        logger.info(
            "Epoch %3d/%d  train loss=%.4f RMSE=%.4f R²=%.4f | val loss=%.4f RMSE=%.4f R²=%.4f | cl=%d lr=%.2e",
            epoch, max_epochs,
            train_loss, train_rmse, train_r2,
            val_loss, val_rmse, val_r2,
            cl_steps, optimizer.param_groups[0]["lr"],
        )
