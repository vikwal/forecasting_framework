#!/usr/bin/env python3
"""
get_test_results_wavenet.py — Evaluate trained Graph WaveNet models on test data.

Pendant to get_test_results_mtgnn.py for the Graph WaveNet baseline (homogeneous
graph, same HomoSampler pipeline as MTGNN — only the model class and config
section ("wavenet" instead of "mtgnn") differ.

Usage
-----
    python geostatistics/get_test_results_wavenet.py \\
        -m wind_wavenet_fold0 \\
        -c configs/wavenet/config_wind_wavenet_fold0.yaml \\
        --hpo-study auto
"""
...
def _setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("eval_wavenet")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Graph WaveNet model on test data")
    ...
    parser.add_argument("--raw-out-name", default=None,
                        help="Override parquet filename stem (without _raw.parquet); "
                             "e.g. wavenet_wind_wavenet_fold0 → data/raw_preds/wavenet_wind_wavenet_fold0_raw.parquet")
    parser.add_argument(
    ...
    _setup_logging()
    logger.info("=== Graph WaveNet Evaluation ===")
    ...
    if args.hpo_study:
        ...
        study_name  = f"cl_m-wavenet_out-{F_h_tmp}_freq-{freq}_{hpo_stem}"
        ...
        if storage_url:
            ...
            db_path = args.hpo_study if args.hpo_study != "auto" else f"studies/hpo_wavenet_{hpo_stem}.db"
