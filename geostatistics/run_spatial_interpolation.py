...
def _guard_target_path(target_path: str, allow_overwrite: bool = False) -> None:
    """Abort if *target_path* already holds TFT-imputed per-station files.

    The wind interpol directory was re-filled on 2026-09-02 by the TFT closing
    model (column 'imputed'); it is the imputation source of every training
    pipeline in this repo and cannot be regenerated by this script. Detecting
    the column is enough -- Kriging output never carries it.
    """
    if allow_overwrite or not os.path.isdir(target_path):
        return
    existing = sorted(glob.glob(os.path.join(target_path, "Station_*.parquet")))
    if not existing:
        return
    try:
        import pyarrow.parquet as pq
        cols = set(pq.ParquetFile(existing[0]).schema_arrow.names)
    except Exception:
        cols = set(pd.read_parquet(existing[0]).columns)
    if "imputed" not in cols:
        return
    raise SystemExit(
        f"Refusing to write into {target_path}: it holds {len(existing)} files from the "
        "TFT closing model (column 'imputed'), the wind_speed imputation source of every "
        "training pipeline (docs/imputation_tft_switch.md). A backup of the previous "
        "Kriging files is at .../synthetic/interpol/wind_vor_tft_20260902. Point "
        "output.target_path somewhere else, or pass --overwrite-imputed if you really "
...
def main() -> None:
    ...
    parser.add_argument(
        "--overwrite-imputed",
        action="store_true",
        help="Allow writing per-station Parquets into a target_path that already "
             "holds files from the TFT closing model (column 'imputed'). Without "
             "this the run aborts before writing -- see docs/imputation_tft_switch.md.",
    )
    ...
    # Guard: since 2026-09-02 the wind target_path (.../synthetic/interpol/wind)
    # holds the TFT closing model's files, which every training pipeline reads
    # as its wind_speed imputation source (docs/imputation_tft_switch.md). A
    # plain run here would silently overwrite them with fresh Kriging output.
    _guard_target_path(target_path, allow_overwrite=args.overwrite_imputed)
