...
#
#   ./launch_tft_bc_hpo.sh <n_gpus>
#
# 8 GPUs (l1) -> 16 workers (8 base / 8 hist)
# 4 GPUs (L2) ->  8 workers (4 base / 4 hist)
#
# NOTE: deliberately no -s/--suffix. hpo_cl_tft_bc.py derives the Optuna study name
...
N_GPUS="${1:?usage: $0 <n_gpus> [max_cache_gb]}"
...
for ((g = 0; g < N_GPUS; g++)); do
    for v in base hist; do
        name="hpo_tft_${v}_g${g}"
        if screen -ls 2>/dev/null | grep -q "\.${name}[[:space:]]"; then
            echo "SKIP  $name (already running)"
            continue
        fi
        screen -dmS "$name" bash -c \
            "source frcst/bin/activate && exec python hpo_cl_tft_bc.py \
             -c configs/tft_bc/config_wind_tft_${v} --gpu ${g} \
             --max-cache-gb ${MAX_CACHE_GB} \
             >> ${LOGDIR}/${name}.log 2>&1"
        echo "START $name  (gpu $g, variant $v)"
        started=$((started + 1))
        sleep 1
    done
done
