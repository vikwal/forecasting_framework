...
| **R6** | **Teil B (Baselines):** MOS (sechs Arme: regional/nearest/local x icond2/both) implementiert, verifiziert (V1-V6/V9 gruen, siehe `docs/baselines_verification_results.md`) und ausgerollt l2->l1->ws am 2026-08-10; QRF-local/HPO/Retrain/QRF-IDW noch offen (naechste Phase). **Teil A (Falsifikationen zur alten Contribution (iv)):** (a) und (c) bewusst gestrichen, (b) der ICON-only-Arm mit `next_n_ecmwf: 0` durch Nutzerentscheidung vom 2026-08-10 GESTRICHEN (nicht mehr Teil des Auftrags) | vor der Auswertung |
